...
def stellaris(
    ...
    while round_done is False:
        if info and info["grad_staleness_mean"] > 0 and not max_staleness:
            max_staleness = int(info["grad_staleness_mean"]) + 1
        
        if max_staleness:
            serverless_bound = np.clip(
                int(max_staleness * (config.algos[algo_name]["serverless_decay"] ** (round_id - 1))) + 1,
                1,
                max_staleness
            )
        else:
        ...
        next_state, next_mask, reward, done, next_info = env.step(
            round_id=round_id,
            action=action,
            serverless_bound=serverless_bound,
        )
        ...
        print("staleness_mean: {}".format(staleness_mean))
        print("decay: {}".format(config.algos[algo_name]["serverless_decay"] ** (round_id - 1)))
        print("serverless_bound: {}".format(serverless_bound))
