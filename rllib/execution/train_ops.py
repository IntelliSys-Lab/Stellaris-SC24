...
def multi_gpu_train_one_step(algorithm, train_batch) -> Dict:
    ...
    if config["async_learn"]:
        num_async_learners_per_device = int(config["num_async_learners"] / num_devices)
        per_device_batch_size = sgd_minibatch_size // num_async_learners_per_device
        batch_size = per_device_batch_size * num_async_learners_per_device
        assert batch_size % num_async_learners_per_device == 0
        assert batch_size >= num_async_learners_per_device, "Batch size too small!"
    else:
            ...
            for _ in range(num_sgd_iter):
                ...
                for batch_index in range(num_batches):
                    ...
                    results = policy.learn_on_loaded_batch(
                        offset=permutation[batch_index] * per_device_batch_size, 
                        buffer_index=0,
                    )

                    if config["async_learn"]:
                        for result in results:
                            learner_info_builder.add_learn_on_batch_results(result, policy_id)
                    else:
                        learner_info_builder.add_learn_on_batch_results(results, policy_id)
            
            if config["async_learn"]:
                policy.reset_async_epoch()
