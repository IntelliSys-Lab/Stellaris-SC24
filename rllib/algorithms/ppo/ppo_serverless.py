"""
Proximal Policy Optimization (PPOServerless)
==================================
...
class PPOServerlessConfig(AlgorithmConfig):
    """Defines a configuration class from which a PPOServerless Algorithm can be built.

    .. testcode::

        from ray.rllib.algorithms.ppo import PPOServerlessConfig
        config = PPOServerlessConfig()
    ...
    def __init__(self, algo_class=None):
        """Initializes a PPOServerlessConfig instance."""
        super().__init__(algo_class=algo_class or PPOServerless)
        ...
        # PPOServerless specific settings:
        ...
        # Override some of AlgorithmConfig's default values with PPOServerless-specific values.
    ...
    def training(
        ...
        **kwargs,
        ...
        Args:
            ...
            clip_param: The PPOServerless clip parameter.
    ...
    def validate(self) -> None:
        ...
        # Synchronous sampling, on-policy/PPOServerless algos -> Check mismatches between
        ...
        if self.sgd_minibatch_size > self.train_batch_size:
            raise ValueError(
                ...
                f"`train_batch_size` ({self.train_batch_size}). In PPOServerless, the train batch"
        ...
        # Episodes may only be truncated (and passed into PPOServerless's
...
class UpdateKL:
    ...
    a `update_kl` method for this to work. This is achieved for PPOServerless via a
...
class PPOServerless(Algorithm):
    def __init__(self, config, *args, **kwargs):
        """Initializes an PPOServerless instance."""
        ...
        # List of AWS Lambda request IDs
        self.aws_request_id_list = []

    @classmethod
    @override(Algorithm)
    def get_default_config(cls) -> AlgorithmConfig:
        return PPOServerlessConfig()
    ...
    def training_step(self) -> ResultDict:
        ...
        # Collect SampleBatches from serverless platform until we have a full batch.
        with self._timers[SAMPLE_TIMER]:
            # Fetch batches from Redis
            unprocessed_sample_batches = self.redis_hget_sample_batch('sample_batch', self.config.serverless_batch_min)
            batches, estimate_batch = self.process_serverless_batches(unprocessed_sample_batches, concat=True)
        ...
        train_results["estimate_batch"] = estimate_batch
    ...
    def redis_hget_sample_batch(self, name, min_batch):
        batch_new = []
        batch_cnt = 0
        while batch_cnt < min_batch:
    ...
    def process_serverless_batches(self, batches, concat):
        processed_multi_agent_batches = []
        processed_sample_batches = []
        ...
        for batch in batches:
            ...
            if batch:
                processed_multi_agent_batches.append(batch)
                processed_sample_batches.append(
                    batch.as_sample_batch().copy()
                )
        if concat:
            processed_multi_agent_batches = concat_samples(processed_multi_agent_batches)
        estimate_batch = concat_samples(processed_sample_batches).copy()

        return processed_multi_agent_batches, estimate_batch
