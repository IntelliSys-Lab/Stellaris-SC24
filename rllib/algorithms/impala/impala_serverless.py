...
class ImpalaServerlessConfig(AlgorithmConfig):
    """Defines a configuration class from which an ImpalaServerless can be built.

    .. testcode::

        from ray.rllib.algorithms.impala import ImpalaServerlessConfig
        config = ImpalaServerlessConfig()
    ...
    def __init__(self, algo_class=None):
        """Initializes a ImpalaServerlessConfig instance."""
        super().__init__(algo_class=algo_class or ImpalaServerless)
...
class ImpalaServerless(Algorithm):
    ...
    def __init__(self, config, *args, **kwargs):
        """Initializes an ImpalaServerless instance."""
    ...
    def training_step(self) -> ResultDict:
        ...
        batches, estimate_batch = self.process_serverless_batches(unprocessed_sample_batches, concat=False)
        ...
        # Samples for hessian estimation
        if self.config.count_steps_by == "agent_steps":
            estimate_batch = synchronous_parallel_sample(
                worker_set=self.workers, max_agent_steps=self.config.estimate_batch_size
            )
        else:
            estimate_batch = synchronous_parallel_sample(
                worker_set=self.workers, max_env_steps=self.config.estimate_batch_size
            )
    ...
    def default_resource_request(
        ...
        if isinstance(config, AlgorithmConfig):
            cf: ImpalaServerlessConfig = config
        else:
            cf: ImpalaServerlessConfig = cls.get_default_config().update_from_dict(config)
